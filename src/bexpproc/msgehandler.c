	   ...
	   ActiveExpInfo.ExpInfo->ExpState = EXPSTATE_STARTED;
	     ...
	     startFID = ActiveExpInfo.ExpInfo->Celem;
   	     ...
   	     DPRINT1(1,"New startAcode value: %d\n",startFID);
              ...
              acqMPS(0);
              int start = startB12proc(ActiveExpInfo.ExpId);
...
int acqHalt(char *args)
{
   ...
   if (abort_in_progress)
   {
	...
	deliverMessage( returnInterface, msge );
      ...
      DPRINT(1, "Abort in process\n");
      return 0;
   }

   strcpy(msge,ActiveExpInfo.ExpInfo->Codefile);
   strcat(msge,"_halt");
   DPRINT1(1, "Rename Code file to %s\n",msge);
   rename(ActiveExpInfo.ExpInfo->Codefile,msge);
   abort_in_progress = 131071;
...
int acqAbort(char *args)
{
   ...
   {
      int fd;
      sprintf(msge,"%s/acqqueue/psg_abort",systemdir);
      if ( (fd = open(msge, O_WRONLY|O_CREAT|O_NOCTTY|O_NONBLOCK, 0666)) >= 0 )
         close(fd);
      setStatGoFlag(-1);	/* No Go,Su,etc acquiring */
      setStatUserId("");
      setStatExpName("");
...
int mpsData(char *args)
{
   ...
   ret = sendMPS(msg);
   if (width == 250)
      strcpy(msg,"rfsweepsw 0\n");
   else if (width == 100)
      strcpy(msg,"rfsweepsw 1\n");
   else if (width == 50)
      strcpy(msg,"rfsweepsw 2\n");
   else
      strcpy(msg,"rfsweepsw 3\n");
   ret = sendMPS(msg);
   sprintf(msg,"rfsweepnpts %d\n", np);
   ret = sendMPS(msg);
